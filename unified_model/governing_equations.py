...
def _get_load_voltage(emf: float, model: UnifiedModel) -> float:
    ...
    return emf * model.load_model.R / (model.load_model.R + model.coil_configuration.get_coil_resistance())
...
def unified_ode(t: Any, y: Any, model: UnifiedModel) -> Any:
    ...
    emf = _get_emf(mag_pos=x3 - x1,
                   mag_vel=x4 - x2,
                   model=model)
    load_voltage= _get_load_voltage(emf=emf,
                                    model=model)
    current = _get_current(emf=emf,
                           model=model)

    if model.coupling_model is not None:
        coupling_force = np.sign(x4 - x2) * model.coupling_model.get_mechanical_force(current)
    else:
