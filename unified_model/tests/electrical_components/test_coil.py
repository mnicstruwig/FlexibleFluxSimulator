...
def test_coil_calc_hover_height():
    ...
    test_fea_data_file_path = os.path.join(
        CUR_DIR,
        '../mechanical_components/test_data/magnetic_spring_fea_data.csv')
    ...
    actual_hover_height = test_coil_config._calc_hovering_height(  # noqa
        magnet_assembly=test_magnet_assembly,
        magnetic_spring=test_magnetic_spring
    )
...
def test_coil_set_optimal_coil_center():
    ...
    test_magnetic_spring = MagneticSpringInterp(
        fea_data_file=test_fea_data_file_path,
        magnet_length=10 / 1000,
        filter_callable=lambda x: savgol_filter(x,
                                           window_length=27,
                                           polyorder=5)
    )

    with patch('unified_model.electrical_components.coil.CoilConfiguration._calc_hovering_height') as mock_calc_hovering_height:  # noqa
        ...
        expected_coil_center = (
            ...
            + 5  # l_epsilon
            + test_n_z * test_coil_wire_radius_mm * 2 / 2  # coil height, divided by 2 (for clarity)
        )
