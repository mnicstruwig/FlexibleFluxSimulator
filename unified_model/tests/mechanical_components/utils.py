...
from unified_model.tests.mechanical_components.test_data import (
    TEST_MAGNET_SPRING_FEA_PATH,
)
...
def build_test_mechanical_system_model():
    ...
    """
    test_spring = MagneticSpring(
        fea_data_file=TEST_MAGNET_SPRING_FEA_PATH, model="coulombs_modified"
    )
    test_magnet_assembly = MagnetAssembly(
        n_magnet=2, l_m=10, l_mcd=5, dia_magnet=10, dia_spacer=10
    )
    ...
    test_footstep = Footstep(
        accelerations=[acc_up, acc_dec, acc_down, acc_impact],
    ...
    test_mechanical_system.set_model(
        "ode_decoupled", initial_conditions=initial_conditions
    )
