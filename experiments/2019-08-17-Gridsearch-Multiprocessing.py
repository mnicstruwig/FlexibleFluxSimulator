...
from unified_model.metrics import dtw_euclid_distance, root_mean_square_percentage_diff
from unified_model.unified import UnifiedModel
from unified_model.utils.utils import (
    build_paramater_grid,
    collect_samples,
...
def make_mechanical_spring(damper_constant):
    return MechanicalSpring(
        position=110 / 1000,  # This must be relative to the top of the _fixed_ magnet!
        strength=1e6,
...
def make_groundtruth_dict(target_time, target_values, label):
    return {"time": target_time, "values": target_values, "label": label}
...
def get_mechanical_groundtruth(which_device, which_sample):
    labeled_video_processor = LabeledVideoProcessor(
        L=125, mm=10, seconds_per_frame=seconds_per_frame, pixel_scale=pixel_scale
    )

    y_target, y_time_target = labeled_video_processor.fit_transform(
        samples[which_device][which_sample].video_labels_df, impute_missing_values=True
    )
...
def get_electrical_groundtruth(which_device, which_sample):
    ...
    adc_processor = AdcProcessor(
        voltage_division_ratio=voltage_division_ratio, smooth=True
    )
...
def build_groundtruth_data(which_device, which_sample, save_to_disk=True):
    ...
    mechanical_groundtruth_dict = get_mechanical_groundtruth(which_device, which_sample)
    electrical_groundtruth_dict = get_electrical_groundtruth(which_device, which_sample)
...
def run_cell(
    ...
    new_unified_model = update_nested_attributes(
        base_unified_model, update_dict=parameter_set
    )
    new_unified_model.solve(
        t_start=0, t_end=8, t_max_step=1e-3, y0=[0.0, 0.0, 0.04, 0.0, 0.0]
    )
    ...
    scores = {"mechanical": m_score, "electrical": e_score}

    curves = {"time": m_eval.time_, "y": m_eval.y_predict_, "emf": e_eval.emf_predict_}
...
def execute_in_batches(
    ...
    for start, stop in indexify(param_grid, batch_size):
        ...
        start_time, current_number = wait_for_completion(
            jobs, start_time, current_number, total=len(param_grid)
        )
...
def wait_for_completion(jobs, start_time=None, current_number=0, total=None):
    ...
    while not done:
        ready, remaining = ray.wait(jobs, num_returns=len(jobs), timeout=5.0)
        num_ready = len(ready)
...
def scores_to_dataframe(scores_dict, param_values_grid, translation_dict):
    """
    ...
    for i, score_dict in enumerate(scores):  # Each score collection
        ...
        for param, param_alt_name in translation_dict.items():
            try:
                accumulated_parameters[param_alt_name].append(
                    param_values_grid[i][param]
                )
            except KeyError:
...
def plot_comparison(
    df_scores, df_curves, time_target, signal_target, signal_column, metric_column, n=1
):
...
metrics = {"mechanical": mechanical_metrics, "electrical": electrical_metrics}
...
for which_sample in which_samples:
    ...
    # Prepare Groundtruth
    # ═════════════════════════════════
    y_groundtruth, emf_groundtruth = build_groundtruth_data(which_device, which_sample)
    ...
    df_scores.to_parquet(
        f"{which_device}_{which_sample}_score.parq",
        engine="pyarrow",
        compression="brotli",
    )
    df_curves.to_parquet(
        f"{which_device}_{which_sample}_curves.parq",
        engine="pyarrow",
        compression="brotli",
    )
