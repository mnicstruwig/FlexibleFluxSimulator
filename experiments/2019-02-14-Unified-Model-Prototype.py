...
accelerometer = AccelerometerInput(
    raw_accelerometer_input=acc_adc_data_path,
    accel_column="z_G",
    time_column="time(ms)",
    time_unit="ms",
    smooth=True,
...
flux_database = FluxDatabase(
    csv_database_path="/home/michael/Dropbox/PhD/Python/Research/fea-flux-curves-numr[5,15]-numz[17,33,66]-wdiam[0.15]-2018-12-07.csv",
    fixed_velocity=0.35,
)

coil_center = {"A": 58.5 / 1000, "B": 61 / 1000}

winding_num_z = {"A": "17", "B": "33"}

winding_num_r = {"A": "15", "B": "15"}

flux_model = flux_database.query_to_model(
...
# POST-PROCESSING
df_result = unified_model.get_result(
    time="t",
    x1="x1",
    x2="x2",
    x3="x3",
...
df_result["emf"] = np.gradient(df_result["x5"].values) / np.gradient(
...
# BEGIN EVALUATION
mech_eval = MechanicalSystemEvaluator(y_target, time_target)
mech_eval.fit(y_predicted, time_predicted)

mech_scores = mech_eval.score(
