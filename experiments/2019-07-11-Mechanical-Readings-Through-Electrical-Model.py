...
from unified_model.utils.testing.testing_electrical_model import simulate_electrical_system, apply_rectification
...
adc_ts = adc_ts
mag_pos, ts = lvp.fit_transform(a_samples[which].video_labels_df,
                                True)
...
new_flux_model = interpolate.interp1d(new_z, phi/squish_factor, fill_value=0, bounds_error=False, kind='quadratic')
new_flux_model = interpolate.interp1d(new_z+z_max, new_flux_model(new_z), bounds_error=False, fill_value=0)
...
t_emf, emf = simulate_electrical_system(mag_pos__,
                                        mag_vel__,
                                        ts__,
                                        new_flux_model,
                                        new_dflux_model,
