...
# Constants
coil_center = {"A": 59 / 1000, "B": 61 / 1000, "C": 63 / 1000}

winding_num_z = {"A": "17", "B": "33", "C": "66"}

winding_num_r = {"A": "15", "B": "15", "C": "5"}

coil_height = {"A": "0.008meter", "B": "0.012meter", "C": "0.014meter"}
...
spring = MagneticSpring(
    fea_data_file=fea_data_path, model="savgol_smoothing", model_type="interp"
)
...
# ELECTRICAL MODEL
flux_database = FluxDatabase(
    csv_database_path="/home/michael/Dropbox/PhD/Python/Research/fea-flux-curves-numr[5,15]-numz[17,33,66]-wdiam[0.15]-cheight[8,12,14]-2019-04-11.csv",
    fixed_velocity=0.35,
)

flux_models = {}
for device in ["A", "B", "C"]:
    flux_models[device] = flux_database.query_to_model(
        flux_model_type="unispline",
...
# BATCH SOLVE
mechanical_evals = {"A": [], "B": [], "C": []}

electrical_evals = {"A": [], "B": [], "C": []}

mechanical_scores = {"A": [], "B": [], "C": []}

electrical_scores = {"A": [], "B": [], "C": []}

unified_models = {"A": [], "B": [], "C": []}
...
for sample_collection in ["A"]:
    for i, sample in enumerate(tqdm(samples_dict[sample_collection])):
        ...
        unified_model.solve(t_start=0, t_end=15, y0=initial_conditions, t_max_step=1e-3)

        # Mechanical scoring
        pixel_scale = 0.18745
        labeled_video_processor = LabeledVideoProcessor(
            L=125, mm=10, seconds_per_frame=3 / 240, pixel_scale=pixel_scale
        )
