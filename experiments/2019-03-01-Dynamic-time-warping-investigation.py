...
def make_square(timestamps, wave_start, wave_end, signal=None):
    magnitude = 1
    signal = [
        magnitude if (t >= wave_start and t <= wave_end) else 0 for t in timestamps
    ]
...
def dtw(x, y, dist="euclidean"):
    dict_dist = {"euclidean": euclidean, "cheb": chebyshev, "mink": minkowski}
    distance, _ = fastdtw(y1, y2, dist=dict_dist[dist])
    return distance


df_noise = run_noise_experiment(
    y1, y2, noise_dict, mse=mean_squared_error, mae=mean_absolute_error, dtw=dtw
)
