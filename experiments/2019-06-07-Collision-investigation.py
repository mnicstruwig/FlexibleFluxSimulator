...
def elastic_spring(x, push_direction="up", pos=0, strength=1000, sharpness=0.01):
    if push_direction is "down":
        ...
        warnings.warn(
            'Incorrect spring direction specified: {}. Using "up"'.format(
                push_direction
            )
        )
...
def inelastic_spring(
    x, v, c, push_direction="up", pos=0, strength=1000, sharpness=0.01
):
    if push_direction is "down":
        direction_modifier = -1
    elif push_direction is "up":
        ...
        raise ValueError(
            f'Incorrect spring direction specified: {push_direction}. \
...
y0 = [0.0, 0]
sol = solve_ivp(
    fun=lambda t, y: bouncing_ball(
        t, y, mass_ball=mass_ball, mechanical_spring=mechanical_spring
    ),
    t_span=t_span,
    y0=y0,
    max_step=1e-3,
