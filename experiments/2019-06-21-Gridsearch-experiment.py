...
mech_spring_coefficients = np.linspace(
    0, 0.25, 5
)  # [0.0125]  # Found from investigation
...
for param_set in tqdm(param_grid):
    new_unified_model = update_nested_attributes(
        base_unified_model, update_dict=param_set
    )
...
def scores_to_dataframe(scores, param_dict, param_values_grid, translation_dict):
    """
...
df = scores_to_dataframe(mech_scores, param_dict, val_grid, translation_dict)
df_elec = scores_to_dataframe(elec_scores, param_dict, val_grid, translation_dict)
df_mv = scores_to_dataframe(mech_v_scores, param_dict, val_grid, translation_dict)
