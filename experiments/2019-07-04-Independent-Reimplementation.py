...
def magnetic_spring_factory(data_path='../unified_model/mechanical_system/spring/data/10x10alt.csv'):
    ...
    # filter signal
    filtered = savgol_filter(df['newtons'],
                             window_length=7,
                             polyorder=3)
...
def flux_factory(data_path='./data/fea-flux-curves/fea-flux-curves-numr[5,15]-numz[17,33,66]-wdiam[0.15]-cheight[8,12,14]-2019-04-11.csv', coil_center=59/1000, velocity=0.35):
    ...
    phi_interp = interp1d(df_A['z'], df_A['phi'], bounds_error=False, fill_value=0, kind='cubic')
    ...
    dphi_interp = interp1d(new_z[1:-1], dphi, bounds_error=False, fill_value=0, kind='quadratic')

    return phi_interp, dphi_interp


def adc_factory(data_path='./data/2019-05-23/A/log_03_adc.csv',
...
sol = solve_ivp(fun=lambda t, y: system(t, y, acc, spring, coupling, phi, dphi_dz, c, m),
                t_span=t_span,
                max_step=1e-3,
                first_step=1e-3,
                y0=y0)
