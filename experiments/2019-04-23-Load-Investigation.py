...
# Path handling
mag_spring_data_path = "./unified_model/mechanical_components/spring/data/10x10alt.csv"

# Note: The accelerometer input is deliberately left out (it is added in later)
spring = MagneticSpring(
    fea_data_file=mag_spring_data_path, model="savgol_smoothing", model_type="interp"
)
...
flux_models = get_flux_models_from_db(
    flux_database,
    ["A", "B", "C"],
...
def _build_quick_unified_model(
    accelerometer_input,
    flux_model,
    load_model,
    coupling_model,
...
accelerometer = AccelerometerInput(
    a_samples[sample_number].acc_df,
    accel_column="z_G",
    time_column="time(ms)",
    smooth=True,
    interpolate=True,
...
unified_model_l = _build_quick_unified_model(
    accelerometer,
    flux_model,
    load_model,
    coupling_model,
...
unified_model_nl = _build_quick_unified_model(
    accelerometer,
    flux_model,
    load_model,
...
unified_model_l.solve(t_start=0, t_end=15, y0=initial_conditions, t_max_step=1e-3)


unified_model_nl.solve(t_start=0, t_end=15, y0=initial_conditions, t_max_step=1e-3)
