    ...
    def make(self):
        return [
            mechanical_components.damper.QuasiKarnoppDamper(
                b_m1, b_m2, self.magnet_assembly, self.tube_inner_radius_mm  # noqa
            )  # noqa
    ...
    def make(self):
        return [
            mechanical_components.MechanicalSpring(
                self.magnet_assembly, self.position, damping_coefficient=dc
            )
            for dc in self.damping_coefficient_list
        ]
...
class CouplingModelFactory:
    ...
    def make(self):
        return [
            CouplingModel().set_coupling_constant(c) for c in self.coupling_factor_list
        ]
    ...
    def make(self):
        ...
        for sample in self.samples_list:
            try:
                ...
                groundtruths.append(Groundtruth(mech_groundtruth, elec_groundtruth))
...
groundtruth_factory = GroundTruthFactory(
    samples_list=samples[which_device],  # noqa
    lvp_kwargs=dict(
...
# Components
input_excitation_factories = {
    device: AccelerometerInputsFactory(samples[device])  # noqa
    for device in ["A", "B", "C"]
}
...
mech_components = {
    ...
    "mechanical_spring": MechanicalSpringFactory(
        magnet_assembly=magnet_assembly,
        position=110 / 1000,
        damping_coefficient_list=np.linspace(0, 10, 5),
    ).make(),
}
...
abstract_model_factory = gridsearch.AbstractUnifiedModelFactory(
    mech_components, elec_components, coupling_models, governing_equations
)
...
curve_expressions = {"t": "time", "x3-x1": "y_diff", "g(t, x5)": "emf"}

# Expressions we want to score
score_metrics = {
    "x3-x1": gridsearch.EvaluatorFactory(
        evaluator_cls=evaluate.MechanicalSystemEvaluator,
        ...
        metrics={
            ...
            "dy_diff_dtw_euclid_norm": metrics.deriv_dtw_euclid_norm_by_length,
        },
    ).make()[
        which_input
    ],  # noqa
    "g(t, x5)": gridsearch.EvaluatorFactory(
        evaluator_cls=evaluate.ElectricalSystemEvaluator,  # noqa
        expr_targets=elec_emf_targets,
        time_targets=elec_time_targets,
        metrics={
            "rms_perc_diff": metrics.root_mean_square_percentage_diff,  # noqa
            "emf_dtw_distance": metrics.dtw_euclid_distance,
            "emf_dtw_euclid_norm": metrics.dtw_euclid_norm_by_length,
        },
    ).make()[
        which_input
    ],  # noqa
